...
키트 부품(단색 LED 빨/노/초 + 흰색 LED)만으로 만드는 발화비율 램프. **LCD1602는 선택**(있으면 숫자 표시 추가).
NeoPixel 없이 **ESP32 PWM 크로스페이드**로 초록→노랑→주황→빨강 그라데이션 효과를 낸다.

> **LCD 없이 LED만으로 충분히 동작합니다.** 기본 설정이 LED 전용(라이브러리 설치 불필요).
> LCD1602(I2C)가 생기면 펌웨어 상단의 `// #define USE_LCD` 주석만 해제하면 숫자 표시가 켜진다.
...
| 220Ω 저항 | 4 |
| 브레드보드 + 점퍼 | — |
| (선택) I2C LCD1602 (QAPASS, 뒤 4핀 백팩) | 0~1 |
...
[선택] LCD1602(I2C):  SDA → GPIO 21
                      SCL → GPIO 22
                      VCC → 5V (VIN 핀)
                      GND → GND
```
- LED 긴 다리(+)가 GPIO 쪽, 짧은 다리(−)가 GND.
- **LCD가 없으면 위 LCD 배선은 건너뛰면 됨** (LED 4개만 연결).
- (LCD 사용 시) 화면이 안 보이면 백팩의 **파란 가변저항(대비)** 을 돌린다. 그래도 안 되면 펌웨어의 `LCD_ADDR`을 `0x27 → 0x3F`로.
...
3. 라이브러리 매니저에서 설치:
   ...
   - (LCD 쓸 때만) **LiquidCrystal I2C** (Frank de Brabander) — `USE_LCD` 정의한 경우에만 필요
...
- **(선택) LCD**: 1행 `L:65%  M:35%`, 2행 16칸 막대(리더 비율). 대기 중엔 `Waiting session`. ASCII만(1602 한글 미지원).

## 6. 빠른 점검
```bash
# 서버 응답 확인 (로컬 BE)
curl http://localhost:8080/api/v1/talk-ratio/active
# → {"data":{"active":false,"leaderRatio":0.0,...}}  미팅 녹음 시작하면 active:true
```
시리얼 모니터(115200): `[Talk] active=1 L:65.0% M:35.0%` 형태 로그 확인.

> 카페 이동 등 **배포(클라우드) BE로 어디서든 쓰려면** → `esp32_talk_ratio_cloud` 스케치 사용 (Railway HTTPS + WiFiManager).
